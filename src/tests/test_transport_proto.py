"""Tests del protocolo CL-transportista (vocabulario ecsdipractica)."""
...
from utilities.namespaces import AGENTS, ECSDI, ECOM, bind_namespaces
...
class TransportProtoTests(unittest.TestCase):
    ...
    def test_cfp_uses_demanar_oferta_transport(self):
        ...
        action = next(msg.subjects(RDF.type, ECOM.DemanarOfertaTransport))
        self.assertEqual(str(next(msg.objects(action, ECOM.comandaId))), "LOT-TEST")
        self.assertEqual(str(next(msg.objects(action, ECOM.producteId))), "P-TEST")
        self.assertEqual(str(next(msg.objects(action, ECOM.ciutatDesti))), "Barcelona")
    ...
    def test_offer_uses_oferta_transport(self):
        ...
        action = next(
            ...
            ).subjects(RDF.type, ECOM.DemanarOfertaTransport)
        )
        ...
        self.assertIn((offer, RDF.type, ECOM.OfertaTransport), offer_msg)
