...
def create_app(agent_uri=DEFAULT_AGENT_URI, logistics_url="http://127.0.0.1:9002/comm"):
    app = Flask(__name__)
    ...
    def comm():
        try:
            ...
            if (action, RDF.type, ECSDI.RealizarPedido) in graph:
                return rdf_response(_handle_order(catalog, agent_uri, message.sender, action, graph, logistics_url))
...
def main():
    ...
    try:
        log(f"tienda-{args.port}", f"listening on {bind_host}:{args.port}, logistics={logistics_url}")
        create_app(logistics_url=logistics_url).run(host=bind_host, port=args.port, debug=False, use_reloader=False)
    finally:
        if registered:
