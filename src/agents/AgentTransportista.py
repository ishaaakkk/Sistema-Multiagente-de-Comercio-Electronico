...
def create_app(
    ...
    def index():
        return (
            ...
            f"ecsdi:DemanarOfertaTransport / ecsdi:OfertaTransport"
        )
    ...
    def comm():
        try:
            ...
            if lote is not None:
                ...
                comanda_id = str(next(graph.objects(action, ECSDI.comandaId), ""))
                product_id = str(next(graph.objects(action, ECSDI.producteId), ""))
                city = str(next(graph.objects(action, ECSDI.ciutatDesti), "Barcelona"))
...
def main():
    parser = argparse.ArgumentParser(description="Transportista externo (vocabulario ECSDI unificado)")
    ...
    transportista_entity = DATA[f"transportista-extern-{args.port}"]
    ...
    registered = register_service(
        ...
        capabilities=[ECSDI.DemanarOfertaTransport],
    )
    try:
        log(
            f"transportista-extern-{args.port}",
